...
?debug=1 ではプレビュー下部に文字配置・フォント調整メニューも表示されます。対象項目は一覧またはCanvas上の枠のクリックで選択でき、位置、範囲、文字サイズ、行間、余白、揃え、端末標準フォントをリアルタイム変更できます。「設定をコピー」で調整結果をJSONとしてコピーし、その値を config.js へ反映してください。開発者メニューの変更はページ再読み込みで初期値へ戻ります。
